...
function startClusterStarter
  ...
  set -l ROCKSDB_OPTS "--dbservers.log.thread true --dbservers.log.time-format timestamp-micros --dbservers.log.level engines=debug --dbservers.log.level threads=trace --dbservers.log.level replication=debug --dbservers.rocksdb.max-write-buffer-number 100"
  # Tell jenkins to not kill this job.
  set -xg BUILD_ID dontKillMe
  eval $STARTER start --starter.wait --starter.data-dir $DATA_PATH --server.js-dir $JS_PATH --server.arangod $ARANGOD_PATH $ENTERPRISE_JS_PATH $JOIN_PART --server.storage-engine $STORAGE_ENGINE $ROCKSDB_OPTS
